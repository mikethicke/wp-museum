...
const Edit = (props) => {
  ...
  const refreshFieldData = () => {
    ...
    apiFetch({ path: `${baseRestPath}/${kindPostType}/fields` }).then(
      setFieldData,
    );
  };
  ...
  return (
    <div>
      <div className="edit-header">
        <h1>{kindLabel}</h1>
        <Button className="do-save-button" onClick={doSave} isPrimary isLarge>
          Save
        </Button>
        {isSaving && (
      ...
      <div className="kind-edit-wrapper">
        <div className="kind-edit">
          <div className="kind-settings" onBlur={saveKindData}>
            <KindSettings
              kindData={kindItem}
              fieldData={fieldData}
              kinds={kinds}
